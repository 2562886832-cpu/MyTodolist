    ...
    <scroll-view class="post-list" scroll-y="true">
      <view class="post-item" v-for="(post, index) in TreeHolePostList" :key="index">
        ...
        <view class="post-header">
		  ...
		  <view class="del" @tap="onDelPost(post.id)">x</view>
        </view>
        ...
        <view class="post-actions">
          ...
          <view class="action-item" @tap="handleLike(post.id)">
            <image 
              class="action-icon" 
              :src="post.liked ? '/static/liked.png' : '/static/like.png'"
            ></image>
            <text class="action-text" :class="{ liked: post.liked }">
              {{ post.likeCount }}
            </text>
          </view>
        </view>
...
<script setup>
	...
	import { apiGetTreeHolePostByUserId } from '../../api/treehole'
	import { apiDelTreeHolePost } from '../../api/mine'
	...
	const getTreeHolePost = async()=>{
		...
		let res = await apiGetTreeHolePostByUserId(queryParams);
	...
	const handleLike = (id) => {
	  if (post.liked) {
	    ...
	    uni.createSelectorQuery()
	      .select(`.post-item:nth-child(${index + 1}) .action-item:nth-child(3)`)
	      .boundingClientRect()
	      .exec(res => {
	        if (res[0]) {
